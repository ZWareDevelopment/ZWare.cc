...
public class PlayerUtil implements Util {
	...
	public static float[] getLegitRotations(Vec3d vec) {
		...
		return new float[]{
				mc.player.rotationYaw + MathUtil.wrapDegrees(yaw - mc.player.rotationYaw),
				mc.player.rotationPitch + MathUtil.wrapDegrees(pitch - mc.player.rotationPitch)
		};
	}

	public static void faceVector(Vec3d vec, boolean normalizeAngle) {
		float[] rotations = PlayerUtil.getLegitRotations(vec);
		mc.player.connection.sendPacket(new CPacketPlayer.Rotation(rotations[0], normalizeAngle ? MathUtil.normalizeAngle((int) rotations[1], 360) : rotations[1], mc.player.onGround));
	}
