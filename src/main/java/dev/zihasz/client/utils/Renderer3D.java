...
public class Renderer3D implements Util {
	...
	public static void drawTextFromBlockWithBackground(BlockPos pos, String text, int color, float scale, Color bColor, boolean border, float width) {
		...
		GlStateManager.translate(-(mc.fontRenderer.getStringWidth(text) / 2.0), 0.0, 0.0);
		if (border)
			Renderer2D.drawRectangleOutline(-1, -1, mc.fontRenderer.getStringWidth(text) + 1, mc.fontRenderer.FONT_HEIGHT - 2, width, bColor);
		Renderer2D.drawRectangle(-1, -1, mc.fontRenderer.getStringWidth(text) + 1, mc.fontRenderer.FONT_HEIGHT - 2, bColor);
		mc.fontRenderer.drawString(text, 0, 0, color);
		enableDepth();
		popMatrix();
	}
