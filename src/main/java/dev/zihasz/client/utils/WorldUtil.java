...
public class WorldUtil implements Util {
	...
	public static List<BlockPos> getPlacePositions(float range, boolean specialCheck, boolean fifteen) {
		...
		List<BlockPos> sphere = getSphere(PlayerUtil.getPlayerPos(), range, (int) range, false, true, 0);
	...
	public static EnumFacing getSide(BlockPos pos) {
		return getSides(pos).stream().findFirst().orElse(null);
	}
